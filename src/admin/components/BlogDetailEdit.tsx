...
export function BlogDetailEdit() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await apiGetRequest(`blogs/${slug}/`);
        if (res.data) {
          const blog = res.data;
        ...
        } else toast.error(res.message || "Failed to load blog");
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await apiPatchRequest(`blogs/${slug}/`, dataToSend);
